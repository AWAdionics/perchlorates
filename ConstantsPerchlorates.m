classdef ConstantsPerchlorates
    properties (Constant)
        Tref = mvu(40,' C')
        Treg = mvu(80,' C')
        cations = {'Li','Ca','Mg' ... %Extracted
                   ,'Na','K'}         %Not Extracted
        anions = {'ClO4',...          %Extracted
                  'Cl','NO3','SO4'}    %Not Extracted
        cations_extracted = {'Li','Ca','Mg'}
        anions_extracted = {'ClO4'}
        c0 = mvu(1000,'mmol/ L')
        y0 = mvu(1000,'mmol/kg_eau')
        Qorg = mvu(6.67e-8,' m^3/ s') %6.67e-7
        Vmix = mvu(1.7e-4,' m^3')
        ak_ext = mvu(0.009,'/ s') %1.8e-2
        ak_rege = mvu(0.0218*8,'/ s') %2*2.18
        c_exctot = mvu(50,'mmol/ L')

        %Process only
        TKapp1 = mvu(40,' C')
        TKapp2 = mvu(80,' C')
        %Kapp1 = mvu(100*[0.5*4.9268e+07,8*8.3334e+12/10,100*2.9709e+08],'')
        Kapp1 = mvu([0.5*4.9268e+07,8*8.3334e+12,2.9709e+08],'')
        %Kapp2 = mvu([17.20692,53.56554,33.33496],'')
        Kapp2 = mvu(5.5*[0.1860,0.5628,0.2512],'') %5 or 6
        %Kapp2 = mvu([1.860,5.628,2.512],'')
        %Kapp2 = mvu([29.9122,628.8899,98.5774],'')
    end
end